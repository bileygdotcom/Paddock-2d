...
class Game:
    ...
    def play(self):
        ...
        for i, lvl in enumerate(s.LEVELS):
            ...
            pygame.mixer.music.load(os.path.join("lib", "music", self.level.song))
    ...
    def wait(self):
        ...
        heading_font = pygame.font.Font(s.FONTS["retro_computer"], 64)
        content_font = pygame.font.Font(s.FONTS["retro_computer"], 32)
        background   = pygame.image.load(os.path.join("lib","title_screen", "high_score.png"))
        heading_text = heading_font.render("High Scores", 1, s.COLOURS["dark_text"])
        y            = 120

        self.window.fill(s.COLOURS["black"])
        self.window.blit(background, (0, 0))
        self.window.blit(heading_text, (60, 3))

        for score in self.high_scores.high_scores:
            date_text  = content_font.render(score[0].strftime("%d %b %Y"), 1, s.COLOURS["dark_text"])
            score_text = content_font.render(str(score[1]), 1, s.COLOURS["dark_text"])

            self.window.blit(date_text, (60, y))
            self.window.blit(score_text, (290, y))
            y += 35
    ...
    def __title_screen(self):
        ...
        pygame.mixer.music.load(os.path.join("lib", "music", "mn84-theme.ogg"))
        pygame.mixer.music.play(-1)

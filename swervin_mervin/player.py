...
class Player:
    ...
    def render_hud(self, window):
        ...
        font        = pygame.font.Font(s.FONTS["retro_computer"], 32)
        ...
        u.render_text("kmph", window, font, s.COLOURS["text"], (110, s.DIMENSIONS[1] - 42))
        u.render_text(str(speed), window, font, s.COLOURS["text"], (10, s.DIMENSIONS[1] - 42))
        ...
        window.blit(p_val_text, (p_val_x, s.DIMENSIONS[1] - 42))
        window.blit(p_name_text, (p_val_x - 112, s.DIMENSIONS[1] - 42))

        # Hit a point milestone.
        if self.points > self.next_milestone and self.status == PlayerStatus.alive:
            milestone_sfx = pygame.mixer.Sound(os.path.join("lib", "music", "excellent.ogg"))
            milestone_sfx.play()
        ...
        # On the leaderboard!
        #print("self.high_score=",self.high_score)
        if self.high_score != None:
            if self.high_score > 0 and self.points > self.high_score:
                high_score_sfx = pygame.mixer.Sound(os.path.join("lib", "music", "excellent.ogg"))
                high_score_sfx.play()
                self.high_score = 0
    ...
    def travel(self, track_length, window):
        ...
        if self.status == PlayerStatus.alive:
            ...
            if self.time_left <= -100:
                go_sfx = pygame.mixer.Sound(os.path.join("lib", "music", "loser.ogg"))
                go_sfx.play()
        ...
        if pos >= track_length:
            ...
            if self.status == PlayerStatus.alive and self.lap == self.total_laps:
                ...
                lap_sfx = pygame.mixer.Sound(os.path.join("lib", "music", "570.wav"))
                lap_sfx.play()
            ...
            if self.status != PlayerStatus.game_over:
                ...
                if self.__fastest_lap():
                    ...
                    if self.lap > 2:
                        ...
                        fast_lap_sfx = pygame.mixer.Sound(os.path.join("lib", "music", "jim.ogg"))
                        fast_lap_sfx.play()
    ...
    def set_acceleration(self, keys):
        ...
        if self.crashed:
            ...
            if (self.x > 1.0 or self.x < -1.0) and self.speed > (self.settings["top_speed"] / self.settings["offroad_top_speed_factor"]):
                ...
                if keys[K_UP] or keys[K_a] or s.AUTO_DRIVE or self.status != PlayerStatus.alive:
                    a = s.FRAME_RATE
                elif keys[K_DOWN] or keys[K_z]:
    ...
    def direction_speed(self):
        return (s.FRAME_RATE * 4 * self.speed_percent())
    ...
    def __hit_bonus(self):
        ...
        bonus_sfx = pygame.mixer.Sound(os.path.join("lib", "music", "oh_yeah.ogg"))
        bonus_sfx.play()
    ...
    def __hit_speed_boost(self):
        if self.speed_boost == 1:
            boost_sfx = pygame.mixer.Sound(os.path.join("lib", "music", "speed_boost.ogg"))
            boost_sfx.play()
    ...
    def __hit_world_object(self):
        ...
        crash_sfx        = pygame.mixer.Sound(os.path.join("lib", "music", "you_fool.ogg"))
        self.crashed     = True
        self.speed       = 0
        self.speed_boost = 1
    ...
    def __hit_competitor(self):
        crash_sfx = pygame.mixer.Sound(os.path.join("lib", "music", "car_crash.ogg"))
        crash_sfx.play()
    ...
    def __run_screech(self):
        if not self.screech_sfx:
            self.screech_sfx = pygame.mixer.Sound(os.path.join("lib", "music", "screech_short.ogg"))
    ...
    def __level_over_overlay(self, window):
        ...
        lo_font      = pygame.font.Font(s.FONTS["retro_computer"], 64)
        s_font       = pygame.font.Font(s.FONTS["retro_computer"], 42)
        ...
        overlay.blit(txt_lo, (s.DIMENSIONS[0] / 2 - txt_lo.get_size()[0] / 2, 120))
        overlay.blit(txt_lap, (s.DIMENSIONS[0]/2 - 200, 280))
        overlay.blit(txt_lap_v, (s.DIMENSIONS[0]/2 + 200 - txt_lap_v.get_size()[0] - 10, 280))
        overlay.blit(txt_bonus, (s.DIMENSIONS[0]/2 - 200, 360))
        overlay.blit(txt_bonus_v, (s.DIMENSIONS[0]/2 + 200 - txt_bonus_v.get_size()[0] - 10, 360))
        overlay.blit(txt_points, (s.DIMENSIONS[0]/2 - 200, 440))
        overlay.blit(txt_points_v, (s.DIMENSIONS[0]/2 + 200 - txt_points_v.get_size()[0] - 10, 440))

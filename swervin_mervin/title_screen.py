...
class TitleScreen():
    ...
    def __init__(self):
        self.finished   = False
        self.ready      = False
        self.background = pygame.image.load(os.path.join("lib","title_screen", "title.png"))
        self.logo_a     = pygame.image.load(os.path.join("lib","title_screen", "title_swervin.png"))
        self.logo_b     = pygame.image.load(os.path.join("lib","title_screen", "title_mervin.png"))
        self.bg_offset  = 0
        self.font       = pygame.font.Font(s.FONTS["retro_computer"], 42)
        self.state      = 0
        self.frame      = 0
        self.logo_a_off = -420
    ...
    def state_1_step(self, window):
        if not self.state == 0:
            ...
            window.blit(self.logo_b, (self.logo_b_off + 170, 150))
            window.blit(self.logo_a, (self.logo_a_off - 170, 150))
    ...
    def state_2_step(self, window):
        if self.state == 2:
            ...
            if (self.frame / 20) % 2 == 1:
                ...
                ic = self.font.render("Press Enter to Start", 1, s.COLOURS["red"])
                x  = (w - ic.get_size()[0]) / 2 + 390
                y  = (h - ic.get_size()[1]) - 5
